<x-app-layout>
    ...
    <div class="space-y-4 animate-fade-in" x-data="{ 
        ...
        selectedIds: [],
        get allSelected() {
            return this.selectedIds.length === {{ $alsintors->count() }} && {{ $alsintors->count() }} > 0;
        },
        toggleAll() {
            if (this.allSelected) {
                this.selectedIds = [];
        ...
        async submitBulkDelete() {
            const { isConfirmed } = await Swal.fire({
                ...
                text: `Anda akan menghapus ${this.selectedIds.length} data Alsintor secara masal!`,
                icon: 'warning',
            ...
            if (isConfirmed) {
                try {
                    const res = await fetch('{{ route('alsintor.bulk-delete') }}', {
                        method: 'POST',
                        headers: {
                ...
                <table class="table-excel">
                    ...
                    <tbody class="divide-y divide-gray-800">
                        @forelse($alsintors as $alsintor)
                            <tr class="transition-colors" :class="selectedIds.includes({{ $alsintor->id }}) ? 'bg-primary-500/5' : ''">
                                <td class="text-center">
                                    <input type="checkbox" :value="{{ $alsintor->id }}" x-model="selectedIds"
                                        class="rounded border-gray-300 bg-white text-primary-600 focus:ring-primary-600 accent-primary-600 transition-all cursor-pointer">
                                </td>
