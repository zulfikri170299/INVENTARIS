    ...
    <div class="space-y-6 animate-fade-in">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-4">
            <div class="glass-card rounded-2xl p-4 text-center border-t-2 border-primary-500">
                ...
                <div class="text-[10px] font-bold text-gray-500 uppercase tracking-wider mt-1">Total</div>
            </div>
            ...
            <div class="glass-card rounded-2xl p-4 text-center border-t-2 border-cyan-500">
                <div class="text-3xl font-black text-cyan-400">{{ number_format($stats['total_r2']) }}</div>
                <div class="text-[10px] font-bold text-gray-500 uppercase tracking-wider mt-1">Roda 2</div>
            </div>
            <div class="glass-card rounded-2xl p-4 text-center border-t-2 border-indigo-500">
                <div class="text-3xl font-black text-indigo-400">{{ number_format($stats['total_r4']) }}</div>
                <div class="text-[10px] font-bold text-gray-500 uppercase tracking-wider mt-1">Roda 4</div>
            </div>
            <div class="glass-card rounded-2xl p-4 text-center border-t-2 border-purple-500">
                <div class="text-3xl font-black text-purple-400">{{ number_format($stats['total_r6']) }}</div>
                <div class="text-[10px] font-bold text-gray-500 uppercase tracking-wider mt-1">Roda 6</div>
            </div>
            <div class="glass-card rounded-2xl p-4 text-center border-t-2 border-pink-500">
                <div class="text-3xl font-black text-pink-400">{{ number_format($stats['total_r8']) }}</div>
                <div class="text-[10px] font-bold text-gray-500 uppercase tracking-wider mt-1">Roda 8</div>
            </div>
        </div>
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div class="flex flex-wrap items-center gap-3">
                <a href="javascript:void(0)"
                    onclick="safeDownload('{{ route('kendaraan.export-excel', request()->all()) }}', 'laporan-kendaraan.xlsx')"
            ...
            <!-- Filter -->
            <form action="{{ route('kendaraan.laporan-ringkas') }}" method="GET"
                ...
                <select name="jenis_roda"
                    ...
                    <option value="R2" {{ ($jenisRoda ?? '') == 'R2' ? 'selected' : '' }}>Roda 2 (R2)</option>
                    <option value="R4" {{ ($jenisRoda ?? '') == 'R4' ? 'selected' : '' }}>Roda 4 (R4)</option>
                    <option value="R6" {{ ($jenisRoda ?? '') == 'R6' ? 'selected' : '' }}>Roda 6 (R6)</option>
                    <option value="R8" {{ ($jenisRoda ?? '') == 'R8' ? 'selected' : '' }}>Roda 8 (R8)</option>
                </select>
                <select name="kondisi"
        ...
        <div class="glass-card rounded-3xl overflow-hidden">
            <div class="px-8 py-5 bg-gradient-to-r from-cyan-600/10 to-indigo-600/10 border-b border-gray-800">
                <h3 class="text-base font-bold text-gray-100 flex items-center">
                    <i class="ph ph-car mr-3 text-xl text-cyan-400"></i>
                    Laporan Ringkas Data Kendaraan per Jenis Roda
                </h3>
                <p class="text-xs text-gray-500 mt-1">Menampilkan {{ $data->count() }} kategori roda dari
                            ...
                            <tr class="hover:bg-gray-800/30 transition-colors">
                                ...
                                <td class="px-6 py-3 border-r border-gray-800/50">
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold
                                            @if($row['jenis_roda'] == 'R2') bg-cyan-500/10 text-cyan-400 ring-1 ring-cyan-500/20
                                            @elseif($row['jenis_roda'] == 'R4') bg-indigo-500/10 text-indigo-400 ring-1 ring-indigo-500/20
                                            @elseif($row['jenis_roda'] == 'R6') bg-purple-500/10 text-purple-400 ring-1 ring-purple-500/20
                                            @else bg-pink-500/10 text-pink-400 ring-1 ring-pink-500/20
                                            @endif">
                                        {{ $row['jenis_roda'] }}
                                    </span>
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-8 py-12 text-center text-gray-500">
                                    <i class="ph ph-car text-4xl mb-3 block opacity-30"></i>
