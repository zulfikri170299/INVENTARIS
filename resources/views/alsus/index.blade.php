<x-app-layout>
    ...
    <div class="space-y-8 animate-fade-in" x-data="{ 
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div class="flex flex-wrap items-center gap-3">
                ...
                <div class="relative" x-data="{ open: false }">
                    ...
                    <div x-show="open" x-transition:enter="transition ease-out duration-100"
                        ...
                        <a href="{{ route('alsus.export-pdf', request()->all()) }}"
                        ...
                        <a href="{{ route('alsus.export-excel', request()->all()) }}"
                            class="flex items-center px-4 py-3 text-sm text-gray-300 hover:bg-gray-700/50 transition-colors">
            ...
            <form action="{{ route('alsus.index') }}" method="GET" class="flex items-center space-x-2">
                <div class="relative">
        ...
        <div class="glass-card p-6 rounded-2xl">
            ...
            @endphp
            <form action="{{ route('alsus.index') }}" method="GET"
                    ...
                    <tbody class="divide-y divide-gray-800 text-sm text-gray-300">
                        @forelse($alsuses as $alsus)
                            <tr class="hover:bg-gray-800/30 transition-colors">
                                ...
                                @if(!auth()->user()->satker_id || in_array(auth()->user()->role, ['Super Admin', 'Super Admin 2']))
                                    <td class="px-8 py-4 text-xs">{{ $alsus->satker->nama_satker ?? '-' }}</td>
                                @endif
                                <td class="px-8 py-4 font-medium text-gray-100">{{ $alsus->jenis_barang }}</td>
                                <td class="px-8 py-4 font-mono text-xs">{{ $alsus->nup ?? '-' }}</td>
                                <td class="px-8 py-4">
                                    @php
                                        $color = match ($alsus->kondisi) {
                                ...
                                <td class="px-8 py-4 text-right">
                                    <div class="flex justify-end space-x-2">
                                        ...
                                        <form id="delete-form-{{ $alsus->id }}"
                                            action="{{ route('alsus.destroy', $alsus->id) }}" method="POST">
                                            @csrf @method('DELETE')
                                            <button type="button"
